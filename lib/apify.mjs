...
function candidatePool() {
  ...
  if (!cachedApifySelection) return candidates;

  const seen = new Set([cachedApifySelection.token]);
  for (const candidate of getApifyCandidates()) {
...
export async function runActorDatasetItems(actorId, payload, { timeoutMs = 30000 } = {}) {
  const candidates = candidatePool();
  if (!candidates.length) throw new Error('Apify token not configured.');
  ...
  for (const candidate of candidates) {
    try {
      start = await fetchJson(
        `${APIFY_API}/acts/${actorRef(actorId)}/runs?token=${encodeURIComponent(candidate.token)}`,
        {
          method: 'POST',
...
export async function runGoogleQueries(queries, { resultsPerPage = 6, timeoutMs = 30000 } = {}) {
  const candidates = candidatePool();
  ...
  return runActorDatasetItems(SEARCH_ACTOR_ID, payload, { timeoutMs });
}
...
function isExcludedEmailDomain(domain = '') {
  return EXCLUDED_EMAIL_DOMAINS.has(domain) || domain.endsWith('.wixpress.com') || domain === 'wixpress.com';
}
...
function extractEmail(input = '') {
  ...
  return isExcludedEmailDomain(domain) ? '' : email;
}
...
export async function fetchPublicWebsiteSignals(url) {
  if (!normalizeWhitespace(url)) {
    return {
      email: '',
      phone: '',
      summary: '',
      contactName: '',
      contactRole: '',
      status: 'skipped-no-website'
    };
  }

  const signals = await fetchWebsiteSignals(url);
  return {
    ...signals,
    status:
      signals.summary || signals.email || signals.phone || signals.contactName || signals.contactRole
        ? 'ok'
        : 'ok-empty'
  };
}
