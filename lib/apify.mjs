...
export async function runGoogleQueries(queries, { resultsPerPage = 6, timeoutMs = 180000 } = {}) {
  ...
  const start = await fetchJson(
    `${APIFY_API}/acts/${actorRef(SEARCH_ACTOR_ID)}/runs?token=${encodeURIComponent(token)}`,
    {
      method: 'POST',
...
export async function enrichLead(lead) {
  ...
  try {
    ...
    return {
      ...
      contact_email: lead.contact_email || websiteSignals.email || '',
      contact_phone: lead.contact_phone || websiteSignals.phone || '',
      contact_name: lead.contact_name || snippetNamed.name || websiteSignals.contactName || '',
      contact_role: lead.contact_role || snippetNamed.role || websiteSignals.contactRole || '',
      public_signals: {
        queries,
