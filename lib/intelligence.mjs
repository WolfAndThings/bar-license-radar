...
function chooseDecisionMaker(record = {}) {
  const choices = [
    ...
    },
    {
      ...
      name: record.enriched_contact_name || record.website_signal_contact_name || '',
      role: record.enriched_contact_role || record.website_signal_contact_role || '',
      email: record.enriched_contact_email || record.website_signal_contact_email || '',
      phone: record.enriched_contact_phone || record.website_signal_contact_phone || ''
    },
    {
      source: 'owner',
...
function classifyAccountType(text = '', primaryType = '') {
  ...
  const barLike = /\btavern\b|\bpub\b|\bsaloon\b|\blounge\b|\bbar\b|\bbar & grill\b|\bgastropub\b/.test(haystack);
  const restaurantLike = /\brestaurant\b|\bgrill\b|\bbistro\b|\bkitchen\b|\bchophouse\b|\bmarket grille\b/.test(haystack);
  ...
  if (restaurantLike && barLike) {
    return { label: 'Restaurant with bar', summary: 'Reads like a restaurant account with a liquor program.' };
  }
  if (barLike) {
    return { label: 'Neighborhood bar', summary: 'Reads like a classic bar, tavern, lounge, pub, or bar-and-grill account.' };
  }
  if (restaurantLike) {
    return { label: 'Restaurant with bar', summary: 'Reads like a restaurant account with a liquor program.' };
  }
  if (/\bevent\b|\bmuseum\b|\barboretum\b|\bvenue\b/.test(haystack) && !barLike && !restaurantLike) {
...
function classifyOpeningStage(lead = {}, marketMatch = null) {
  ...
  if ((fit === 'New issuance' || fit === 'New / timely') && future) {
    return {
      label: 'Pre-open / licensing',
      summary: 'This is a live license filing or hearing window. Treat it as a pre-open signal, not proof the bar is already operating.'
    };
  }

  if ((fit === 'New issuance' || fit === 'New / timely') && ageDays !== null && ageDays <= 45) {
    if (marketMatch && daysSince(marketMatch.oldest_visible_review_date || '') !== null && daysSince(marketMatch.oldest_visible_review_date || '') <= 90) {
      return {
        label: 'Just opened',
        summary: 'The license record is recent and the market profile already shows live public activity, so this may have just opened.'
      };
    }
    return {
      label: 'Fresh license activity',
      summary: 'The filing is recent, but licensing usually lands before opening. Treat this as a fresh pre-open signal until market or website evidence says the venue is operating.'
    };
  }
  ...
  return {
    label: future ? 'In licensing' : 'Fresh activity',
    summary: 'This is still a live public-record signal worth checking, but it should not be treated as proof the venue is already open.'
  };
}
...
function timingScore(record = {}, openingStage = {}, hiring = {}) {
  let score = 22;
  if (openingStage.label === 'Pre-open / licensing') score += 48;
  else if (openingStage.label === 'Just opened' || openingStage.label === 'Ownership change') score += 36;
  else if (openingStage.label === 'Fresh license activity') score += 24;
