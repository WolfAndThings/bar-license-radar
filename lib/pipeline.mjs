...
import {
  collectDuluthLeads,
  collectFargoLeads,
  collectMinneapolisLeads,
  collectSiouxFallsLeads,
  collectStPaulLeads
} from './collectors.mjs';
...
import { buildMarketInventory } from './market.mjs';
import { enrichLeadRecords, enrichMarketRecords } from './intelligence.mjs';
...
const MARKET_PATH = path.join(DATA_DIR, 'market.json');
...
export async function runRefresh() {
  ...
  try {
    const duluth = await collectDuluthLeads({ daysBack: JUST_MISSED_WINDOW_DAYS });
    collected.push(...duluth);
  } catch (error) {
    warnings.push(`Duluth collector failed: ${error.message}`);
  }
  ...
  for (const lead of deduped) {
    ...
    const finalLead = {
      ...
      board_bucket: isTargetOpportunity(analysis) ? 'opportunity' : 'watchlist'
    };
    activity.push(finalLead);
  ...
  const marketInventory = await buildMarketInventory(activity).catch((error) => ({
    records: [],
    meta: {
      actor_id: '',
      market_count: 0,
      warnings: [`Market inventory failed: ${error.message}`]
    }
  }));
  warnings.push(...(marketInventory.meta?.warnings || []));

  const intelligentMarkets = enrichMarketRecords(marketInventory.records, activity, activity);
  const intelligentActivity = enrichLeadRecords(activity, intelligentMarkets, activity).map((lead) => {
    const suggested = buildSuggestedFollowUp(lead);
    return {
      ...lead,
      suggested_follow_up_subject: suggested.subject,
      suggested_follow_up_body: suggested.body
  ...
  const retained = intelligentActivity.filter((lead) => isTargetOpportunity(lead));

  const meta = {
    generated_at: new Date().toISOString(),
    lead_count: retained.length,
    activity_count: intelligentActivity.length,
    market_count: intelligentMarkets.length,
    cities: [...new Set(retained.map((lead) => lead.source_city))],
    apify,
    propertyRadar,
    marketInventory: {
      ...marketInventory.meta,
      market_count: intelligentMarkets.length
    },
    warnings
  };

  await fs.writeFile(LEADS_PATH, JSON.stringify(retained, null, 2));
  await fs.writeFile(ACTIVITY_PATH, JSON.stringify(intelligentActivity, null, 2));
  await fs.writeFile(MARKET_PATH, JSON.stringify(intelligentMarkets, null, 2));
