...
function isLicenseIssuanceSignal(text = '') {
  return /\bissuance of (?:an?|a)\b|\bnew issue\b|\blicense application summary\b/i.test(text);
}

function isTemporaryPermitSignal(text = '') {
  return /\bspecial one-day liquor license\b|\bone-day liquor\b|\btemporary permit\b|\bspecial event\b/i.test(text);
}

function isConsumptionOnlySignal(text = '') {
  return /\bconsume, but not sell, alcoholic beverages\b|\balcohol consumption request\b/i.test(text);
}

function describeHearingPurpose(lead, text) {
  const business = lead.business_name || 'this business';

  if (isConsumptionOnlySignal(text)) {
    return `The hearing is for ${business} to allow alcohol consumption at a specific event or venue without retail alcohol sales.`;
  }

  if (isTemporaryPermitSignal(text)) {
    return `The hearing is for a temporary or one-day liquor permit tied to ${business}, not a durable everyday bar account by itself.`;
  }
  ...
  if (isLicenseIssuanceSignal(text)) {
    return `The public record appears to be for the issuance of a liquor license tied to ${business}.`;
  }
...
export function analyzeLead(lead) {
  ...
  if (lead.source_city === 'Fargo') {
    reasons.push(
      `Included because the Fargo Liquor Control Board minutes recorded a liquor-license application or issuance item for ${lead.business_name || 'this business'}.`
    );
    if (lead.address) {
      reasons.push(`The Fargo minutes exposed a location at ${lead.address}.`);
    }
  }

  if (lead.source_city === 'Sioux Falls') {
    reasons.push(
      `Included because the Sioux Falls City Council agenda contained an alcohol-related item for ${lead.business_name || 'this business'}.`
    );
    if (isTemporaryPermitSignal(text) || isConsumptionOnlySignal(text)) {
      reasons.push('This Sioux Falls item reads like a temporary event-use request, so it is being treated as a weak signal rather than a core target account.');
    }
  }
  ...
  if (isLicenseIssuanceSignal(text)) {
    score += 12;
    salesReasons.push('The public text explicitly describes license issuance or an application summary, which is stronger than a generic notice.');
  }
  ...
  if (isTemporaryPermitSignal(text)) {
    score -= 34;
    salesReasons.push('This looks like a temporary or one-day permit rather than a durable retail account.');
  }

  if (isConsumptionOnlySignal(text)) {
    score -= 24;
    salesReasons.push('The record allows alcohol consumption without retail sales, which is a weak distributor target.');
  }
  ...
  let fit = 'Mixed';
  if (isConsumptionOnlySignal(text)) fit = 'Temporary / consumption-only';
  else if (isTemporaryPermitSignal(text)) fit = 'Temporary / event permit';
  else if (isGenericMinneapolisCode(text)) fit = 'Weak signal / generic code';
  else if (isOwnershipSignal(text)) fit = 'Ownership / operator change';
  else if (isLicenseIssuanceSignal(text) && !isLikelyExistingVenue(text)) fit = 'New issuance';
