...
function contactEntryText(entry) {
  ...
  return (
    normalizeContactPrimitive(entry.linktext) ||
    normalizeContactPrimitive(entry.Linktext) ||
    normalizeContactPrimitive(entry.value) ||
    normalizeContactPrimitive(entry.Value) ||
    normalizeContactPrimitive(entry.href) ||
    normalizeContactPrimitive(entry.Href)
  );
}
...
function summarizeProperty(property = {}, primaryPerson, comps = []) {
  ...
  const primaryPhone = firstContactText(primaryPerson, ['PhoneValues', 'Phone']);
  const primaryEmail = firstContactText(primaryPerson, ['EmailValues', 'Email']);
...
export async function hydratePropertyRadar(lead) {
  ...
  try {
    ...
    const ownerPhone =
      firstContactText(primaryPerson, ['PhoneValues', 'Phone']) ||
      people.map((person) => firstContactText(person, ['PhoneValues', 'Phone'])).find(Boolean) ||
      '';
    const ownerEmail =
      firstContactText(primaryPerson, ['EmailValues', 'Email']) ||
      people.map((person) => firstContactText(person, ['EmailValues', 'Email'])).find(Boolean) ||
      '';
